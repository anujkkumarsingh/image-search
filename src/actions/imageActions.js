...
export const getImages = (new_query) => (dispatch, getState) => {
  ...
  axios
    .get(
      `https://api.unsplash.com/search/photos/?page=1&per_page=${per_page}&query=${query}&client_id=QWssdAlKrbUo-apkXbfT_EYGDZnfHWVsfYOYAIxWLws`
    )
    .then((res) => {
      dispatch({
        type: GET_IMAGES,
...
export const loadMore = () => (dispatch, getState) => {
  ...
  axios
    .get(
      `https://api.unsplash.com/search/photos/?page=${page_num}&per_page=${per_page}&query=${query}&client_id=QWssdAlKrbUo-apkXbfT_EYGDZnfHWVsfYOYAIxWLws`
    )
    .then((res) => {
      dispatch({
        type: LOAD_MORE,
